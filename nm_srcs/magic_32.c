...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/12 12:25:16 by pnardozi          #+#    #+#             */
/*   Updated: 2019/07/12 12:25:29 by pnardozi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_nm.h"

static void		handle_sect_symbol_32(t_env *env, t_symb *tmp,
		char symbol, struct nlist *ptr)
{
	if (symbol == 0x0)
		tmp->symb = 'U';
	else if (symbol == 0xC)
...
static void		handle_type_32(t_env *env, struct nlist *ptr,
		char *string, uint32_t (*indian)(uint32_t))
{
	...
	if ((ptr->n_type & 0xE0) == 0)
	{
		if (!(tmp = malloc(sizeof(t_symb))))
			return ;
		tmp->val = indian(ptr->n_value);
		handle_sect_symbol_32(env, tmp, symbol, ptr);
		if ((ptr->n_type & 0x1) == 0)
			tmp->symb += 32;
...
static void		construct_list_32(t_env *env, uint32_t (*indian)(uint32_t))
{
	uint32_t			i;
	char				*string;
	struct nlist		*ptr;
	...
	i = 0;
	if (!(ptr = (struct nlist *)(move_ptr(env, env->file,
						(size_t)indian(env->sym->symoff) + env->offset))))
		return ;
	if (!(string = (char*)(move_ptr(env, env->file,
					(size_t)indian(env->sym->stroff) +
					(size_t)(indian(ptr->n_un.n_strx)) + env->offset))))
		return ;
	while (i < indian(env->sym->nsyms))
	{
		handle_type_32(env, ptr, string, indian);
...
static void		get_sections_number_32(t_env *env, uint32_t (*indian)(uint32_t))
{
	uint32_t					n_sects;
	struct segment_command		*cmd;
	struct section				*sect;

	cmd = (struct segment_command *)(void *)(env->current_lc);
...
void			handle_32(t_env *env, uint32_t (*indian)(uint32_t))
{
	...
	if (!(header = (struct mach_header *)(move_ptr(env,
						env->file, env->offset))))
		return ;
	...
	while (i < indian(header->ncmds))
	{
		...
		if (indian(env->current_lc->cmd) == LC_SEGMENT)
			get_sections_number_32(env, indian);
		i++;
		if (!(env->current_lc = (struct load_command *)(move_ptr(env,
				env->current_lc, (size_t)indian(env->current_lc->cmdsize)))))
			return ;
	}
	construct_list_32(env, indian);
}
