...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/12 12:28:15 by pnardozi          #+#    #+#             */
/*   Updated: 2019/07/12 12:28:16 by pnardozi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			print_sect_32(t_env *env, uint32_t (*indian)(uint32_t))
{
	size_t		i;
	size_t		size;
...
static void		get_section_text_32(t_env *env, uint32_t (*indian)(uint32_t))
{
	uint32_t				n_sects;
	...
	while (n_sects > 0)
	{
		if (ft_strcmp(env, sect->sectname, "__text") == 0)
		{
			...
			env->text_addr = indian(sect->addr);
...
void			handle_32(t_env *env, uint32_t (*indian)(uint32_t))
{
	...
	while (i < indian(header->ncmds))
	{
		if (indian(env->current_lc->cmd) == LC_SEGMENT)
			get_section_text_32(env, indian);
		i++;
		if (!(env->current_lc = (struct load_command *)(move_ptr(env,
